...
{
	int FirstChannel;      // 首通道[0, 31]
	int LastChannel;       // 末通道[0, 31],要求末通道必须大于或等于首通道
	int InputRange;		// 模拟量输入量程范围,其取值请见下面相关常量定义
	int GroundingMode;		// 接地方式(单端或双端选择),其取值请见下面相关常量定义
	int Gains;
...
//################# 内存映射寄存器直接操作及读写函数 ########################
    ...
    BOOL DEVAPI FAR PASCAL PCI8735_WritePortByte(HANDLE hDevice, PUCHAR pbPort, ULONG offserBytes, BYTE Value);
    BOOL DEVAPI FAR PASCAL PCI8735_WritePortWord(HANDLE hDevice, PUCHAR pbPort, ULONG offserBytes, WORD Value);
    BOOL DEVAPI FAR PASCAL PCI8735_WritePortULong(HANDLE hDevice, PUCHAR pbPort, ULONG offserBytes, ULONG Value);

    BYTE DEVAPI FAR PASCAL PCI8735_ReadPortByte(HANDLE hDevice, PUCHAR pbPort, ULONG offserBytes);
    WORD DEVAPI FAR PASCAL PCI8735_ReadPortWord(HANDLE hDevice, PUCHAR pbPort, ULONG offserBytes);
    ULONG DEVAPI FAR PASCAL PCI8735_ReadPortULong(HANDLE hDevice, PUCHAR pbPort, ULONG offserBytes);
